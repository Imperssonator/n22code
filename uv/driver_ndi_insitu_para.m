datadir = 'data/insitu_n2200_para_015_2';
spec_start = 100;
spec_stop = 800;
...
wavestart=400;
wavestop=1000;
wavestart_ind = find( uvs(1).waves_para>wavestart, 1 );
wavestop_ind = find( uvs(1).waves_para>wavestop,1 );
...
paramat = paramat(wavestart_ind:wavestop_ind,:);
perpmat = [ uvs(spec_start:spec_stop).abs_perp ];
perpmat = perpmat(wavestart_ind:wavestop_ind,:);
